...
async function processBatch(batch, client, existingCodes, stats) {
...
async function main() {
  console.log("── Maharashtra GR Index update ────────────────────");
  ...
  const { rows } = await client.query("SELECT code FROM gr_index");
  const existingCodes = new Set(rows.map(r => r.code));
  console.log(`   Loaded ${existingCodes.size} existing GR codes`);
  console.log("   Connected ✓");



  // 4. Seed in batches
  console.log(`\n3. Seeding ${files.length} GRs for updates(${CONCURRENCY} at a time)...\n`);
  const stats = { done: 0, skipped: 0, failed: 0, total: files.length };

  for (let i = 0; i < files.length; i += CONCURRENCY) {
    const batch = files.slice(i, i + CONCURRENCY);
    await processBatch(batch, client, existingCodes, stats);
    if (i + CONCURRENCY < files.length) await sleep(DELAY_MS);
  }

  // 5. Summary
const { rows: countRows } = await client.query(
    "SELECT COUNT(*) FROM gr_index"
);
